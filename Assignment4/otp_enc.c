 ...
 * Author: Navine Rai
 * Date: 2-26-20
 * Description: This program connects to daemon otp_enc_d
 * 		and asks it to perform a one-time pad
 * 		encryption on the file that it is passed
 * 		when run.
 **********************************************************/
...
int main(int argc, char* argv[]) {
	int socketFD, fileFD, keyFD, portNumber, charsWritten, charsRead, keyLength, plaintextLength;
	...
	//read the contents of the passed in plaintext file
	strcpy(filePath, "./");
	...
	nPlainRead = read(fileFD, fileBuffer, sizeof(fileBuffer));
	plaintextLength = nPlainRead - 1;
	...
	//determine if the plaintext file has bad characters
	for (i = 0; i < plaintextLength; i++) {
		...
		if (goodChar == 0) { error("ERROR: found bad characters in plaintext file\n"); }
	}
	...
	if (keyLength < plaintextLength) { error("CLIENT: Key is not long enough\n"); }

	//send plaintext to server
	charsWritten = send(socketFD, fileBuffer, strlen(fileBuffer), 0);
	//printf("%d characters of plaintext were just sent\n", charsWritten);
	...
	//printf("%d characters of key were just sent\n", charsWritten);
	if (charsWritten < 0) error("CLIENT: ERROR writing to socket");
